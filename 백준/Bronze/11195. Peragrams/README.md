# [Bronze II] Peragrams - 11195

[문제 링크](https://www.acmicpc.net/problem/11195)

### 성능 요약

시간 제한: 1 초, 메모리 제한: 256 MB

### 통계

제출: 376, 정답: 285, 맞힌 사람: 266, 정답 비율: 75.568%

### 분류

구현, 문자열, 애드 혹, 홀짝성

### 문제 설명

<p>Per recently learned about palindromes. Now he wants to tell us about it and also has more awesome scientific news to share with us.</p>

<p>&ldquo;A palindrome is a word that is the same no matter whether you read it backward or forward&rdquo;, Per recently said in an interview. He continued: &ldquo;For example, add is not a palindrome, because reading it backwards gives dda and it&rsquo;s actually not the same thing, you see. However, if we reorder the letters of the word, we can actually get a palindrome. Hence, we say that add is a Peragram, because it is an anagram of a palindrome&rdquo;.</p>

<p>Per gives us a more formal definition of Peragrams: &ldquo;Like I said, if a word is an anagram of at least one palindrome, we call it a Peragram. And recall that an anagram of a word w contains exactly the same letters as w, possibly in a different order.&rdquo;</p>

<p>Given a string, find the minimum number of letters you have to remove from it, so that the string becomes a Peragram.</p>

### 입력

<p>Input consists of a string on a single line. The string will contain at least 1 and at most 1 000 characters. The string will only contain lowercase letters a-z.</p>

### 출력

<p>Output should consist of a single integer on a single line, the minimum number of characters that have to be removed from the string to make it a Peragram.</p>