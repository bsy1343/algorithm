...
제출: 224, 정답: 189, 맞힌 사람: 158, 정답 비율: 86.813%
...
<p style="text-align:center">3, 10, 17, 24, 31, &hellip;</p>
...
<p>If it is possible to complete the sequence with integers, then output ten integers on a single line, with a single space between them, by replacing the 0 values with the correct numbers. If it is not possible to complete the sequence with integers, simply output a single -1. Although the two non-zero inputs are positive, the rest of the sequence might not be. Likewise, while the two non-zero inputs are &le;1,000, the rest of the sequence might not be.</p>