...
제출: 2750, 정답: 1235, 맞힌 사람: 1042, 정답 비율: 43.874%
...
<p>첫 번째 줄에 테스트 케이스의 개수 $T$ ($1 &le; T &le; 10^4$)가 주어진다.</p>

<p>각 테스트 케이스는 한 줄로 구성되어, 두 정수 $x$, $y$ ($0 &le; x, y &le; 99$)가 공백 하나로 구분되어 주어진다.</p>

### 출력

<p>각 테스트케이스 마다 두 문자열을 공백 하나로 구분하여 출력한다. 첫 번째 문자열은 두 정수 $x$, $y$를 $x$시 $y$분으로 읽는 것이 가능하면 &ldquo;<code>Yes</code>&rdquo;, 가능하지 않으면 &ldquo;<code>No</code>&rdquo;여야 한다. 두 번째 문자열은 두 정수 $x$, $y$를 $x$월 $y$일로 읽는 것이&nbsp;가능하면 &ldquo;<code>Yes</code>&rdquo;, 가능하지 않으면 &ldquo;<code>No</code>&rdquo;여야 하다.</p>
