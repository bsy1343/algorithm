...
제출: 362, 정답: 314, 맞힌 사람: 272, 정답 비율: 88.312%
...
<p>The first input line contains two integers: n (1 &le; n &le; 30), indicating the number of team members and r (1 &le; r &le; 50,000), indicating the fixed raise offered by FAAMGInc. Each of the next n input lines contains an integer (between 1 and 200000, inclusive) indicating the current salary for a team member.</p>
...
<p>If a team member will end up with the same raise choosing either option, that team member doesn&rsquo;t count towards either group. For example, if the fixed raise is \$2000 and a team member&rsquo;s current salary is \$2000, the fixed raise and double the salary end up with the same raise for this team member so the team member doesn&rsquo;t count towards either group.</p>

### 힌트

<p>The financial statements in this problem are fictional.</p>