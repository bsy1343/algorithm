...
제출: 5067, 정답: 3821, 맞힌 사람: 3604, 정답 비율: 76.114%
...
<p>The input consists of a single line, containing $8$ integers $N_1$, $N_2$, $N_3$, $N_4$, $N_5$, $N_6$, $N_7$ and $N_8$, indicating the values read by the device ($N_i$ is <code>0</code>, <code>1</code> or <code>9</code> for $1 &le; i &le; 8$).</p>

### 출력

<p>Print a single line containing the capital letter &ldquo;<code>S</code>&rdquo; if all bits are read successfully; otherwise print a single line containing the capital letter &ldquo;<code>F</code>&rdquo;, corresponding to a failure.</p>