...
제출: 162, 정답: 110, 맞힌 사람: 97, 정답 비율: 68.310%
...
<p>The first line of a data set contains three integers a single integer 0 &le; n &le; 30: the total number of water main breaks. The next line contains two integers 0 &le; s &le; f &le; 1000. [s, f] is the interval (in seconds) during which you are to compute the water wasted; s is the start time, and f is the finish time. This is followed by n lines, each describing a water main break. Each line i contains three integers s<sub>i</sub>, f<sub>i</sub>, r<sub>i</sub>. 0 &le; s<sub>i</sub> &le; f<sub>i</sub> &le; 1000 are the start and finish time of the water main break, in seconds. 0 &le; r<sub>i</sub> &le; 1000 is the rate at which water flows out of the pipe, given in gallons per second.</p>

<p>Notice that the water main break is fixed at time fi+1, so that ri gallons of water leave at times s<sub>i</sub>, s<sub>i+1</sub>, s<sub>i+2</sub>, . . . , f<sub>i&minus;1</sub>, f<sub>i</sub></p>

### 출력

<p>For each data set, output &ldquo;Data Set x:&rdquo; on a line by itself, where x is its number.</p>

<p>Then, on a line by itself, print the total number of gallons of water that have been wasted during the interval [s, f]. Notice that we thus count water that is leaking at times s, s + 1, . . . , f &minus; 1, f.</p>
