...
제출: 99, 정답: 73, 맞힌 사람: 67, 정답 비율: 79.762%
...
<p>The first line of a data set contains three numbers c, d,t with 1 &le; c &le; 100, 1 &le; d &le; 1000, 1 &le; t &le; 100000. Here, c is the number of political candidates (numbered 1, . . . , c), d the number of donors (numbered 1, . . . , d), and t the number of transactions.</p>
...
<p>For each data set, first output &ldquo;Data Set x:&rdquo; on a line by itself, where x is its number. Then, output either the line &ldquo;No violations&rdquo; on a line by itself, or &ldquo;Violators:&rdquo; on a line by itself, followed by all the donors who violated one of the limits, each on a line by himself. The donors are to be sorted by increasing number.</p>