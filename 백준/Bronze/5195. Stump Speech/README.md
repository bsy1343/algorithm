...
제출: 187, 정답: 69, 맞힌 사람: 59, 정답 비율: 36.875%
...
<p>Wouldn&rsquo;t it be much easier if we could save ourselves the trouble of getting an actual focus group, and instead had a computer program that would tell us how people would react to a speech? Here, we are going to write a program that gives a first rudimentary approximation to this functionality. You will be given a list of key words and phrases, and the reactions they elicit. Based on those, you are to assess the quality of the speech.</p>

<p>More specifically, each phrase has a positive or negative score attached with it, for instance, &ldquo;taxes&rdquo; could have a score of -2, &ldquo;increase taxes&rdquo; a score of -4, &ldquo;education&rdquo; a score of 3, and &ldquo;slash education&rdquo; a score of -6. You would then count the number of times each of these phrases occurs in the text, and add the corresponding scores, to obtain the score of the speech.</p>

<p><sup>3</sup>perhaps fibbing a little &mdash; even the good old times were not that good.</p>
...
<p>This is followed by one more line of at most 10000 characters, containing the candidate&rsquo; speech. This string consists only of lowercase letters, white space, and the punctuation signs &lsquo;.&rsquo; and &lsquo;,&rsquo;. (In the example below, there are line breaks, in order to display the input properly. There won&rsquo;t be any line breaks as part of the text in the actual test files.)</p>

### 출력

<p>For each data set, first output &ldquo;Data Set x:&rdquo; on a line by itself, where x is its number. Then, output the score of the proposed text on a line by itself. The score is the sum over all key phrases that occur in the text (if they occur multiple times, they are scored multiple times). Only exact matches count &mdash; if as much as the amount of white space is different, we assume that it is not the key phrase.</p>