...
제출: 125, 정답: 102, 맞힌 사람: 77, 정답 비율: 78.571%
...
<p>Each test case is given using exactly two lines. The first line contains two integers N and B. The meaning of N was described above (1 &le; N &le; 90), while B represents the number of balls which remained in the bag (2 &le; B &le; N + 1). The second line contains B distinct integers b<sub>i</sub>, indicating the balls which remained in the bag (0 &le; b<sub>i</sub> &le; N).</p>

<p>The last test case is followed by a line containing two zeros.</p>

### 출력

<p>For each test case output a single line containing a single uppercase &lsquo;Y&rsquo; if is possible to call out every number from 0 to N, inclusive, or a single uppercase &lsquo;N&rsquo; otherwise.</p>