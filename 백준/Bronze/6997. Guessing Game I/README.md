...
제출: 184, 정답: 108, 맞힌 사람: 94, 정답 비율: 66.197%
...
<ul>
	<li><strong>secret</strong> = 1234, <strong>guess</strong> = 1357: The &ldquo;1&rdquo; in <em>guess</em> gives us one circle, whereas the &ldquo;3&rdquo; in <em>guess</em> gives us one square. The other two digits in <em>guess</em> do not have a match.</li>
</ul>
...
<ul>
	<li><strong>secret</strong> = 1234, <strong>guess</strong> = 1122: There is only one exact match, for position 1 (that gives us one circle). The second &ldquo;1&rdquo; in guess doesn&rsquo;t match anything in secret (since the &ldquo;1&rdquo; in secret is already matched). Then, we get one square for the first &ldquo;2&rdquo; in guess, and that&rsquo;s it.</li>
	<li><strong>secret</strong> = 1311, <strong>guess</strong> = 1122: One circle for exact match at position 1, and one square for an out-of-position match for digit &ldquo;1&rdquo;.</li>
...
<p>The first line in the test data file contains the number of test cases (&lt; 100). After that, each line contains one test case: the first number is the <em>secret</em> (provided as an int), and the following number is the <em>guess</em> (provide as an int). Both the numbers are &le; 9999. Note that the numbers need to be padded with 0&rsquo;s. For example, if one of the numbers is 1, it needs to be treated as 0001.</p>
