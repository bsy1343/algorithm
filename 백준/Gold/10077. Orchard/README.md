...
제출: 38, 정답: 20, 맞힌 사람: 15, 정답 비율: 75.000%
...
<p>Alex would like to keep all apple trees and Bert would like to keep all banana trees, but do not want to replant any tree. When they talked to a lawyer, the lawyer informed them that the ownership of a tree can be transferred from one owner to another, but the lawyer would charge \$1 to transfer the ownership of a tree. Therefore the brothers try to place the starting rectangle such that the legal fees are as low as possible.</p>
...
<p>In the first example, the best way is to cut out the fourth column and to assign it to Bert, as indicated by the rectangular outline. Afterwards, there are two trees that are out of place, and their ownerships have to be transferred &ndash; one banana tree from Alex to Bert and one apple tree from Bert to Alex. Hence, the fees are \$2.</p>

<p>In the second example, the best way is to cut out trees that are not at the border of the field and then to transfer the ownership of six trees. The fees are \$6.</p>

<p>In the third example, the best way is to cut out the rectangle consisting of 3 banana trees, and then to transfer the ownership of the rightmost banana tree to Bert. The fee is \$1. Note that there is an alternative way that costs \$1 as well.</p>

### 입력

<p>Your program must read from standard input. The first line of the input are the two positive numbers n and m indicating the orchard&rsquo;s size. Then follow n lines each consisting of m numbers, where 0 represents an apple tree and 1 represents a banana tree.</p>
