...
제출: 53, 정답: 23, 맞힌 사람: 21, 정답 비율: 55.263%
...
may be assumed to be in strictly increasing order.
    ...
    While Low &lt;= High
        ...
        If a[Mid] &gt; x then High = Mid &ndash; 1
...
<p>For each input case, print the total loop count to find all the values in a list of size n. Follow this format exactly: &ldquo;Case&rdquo;, one space, the case number, a colon and one space, and the answer for that case with no trailing spaces. (see next page for sample)</p>