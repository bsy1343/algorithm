...
제출: 73, 정답: 37, 맞힌 사람: 31, 정답 비율: 51.667%
...
<p>Sometimes, a neural net can go haywire and become what is more commonly known as a neurotic network. Consider this your chance to launch a second career in psychiatry. The scenario is that someone just came in with a neurotic network in their head. What this means is that if the output of their neural net is an even number, the person will freak out and will set fire to a kitten. Therefore, it is of vital importance that you can know ahead of time whether or not a given person is safe. If it is safe, print their neural output modulo 1,000,000,007. If you wouldn&rsquo;t trust the person to be around kittens who&rsquo;re not wrapped in fire retardant, print the string &ldquo;FREAK OUT&rdquo; (without the quotes).</p>

### 입력

<p>The first line of input is T, the number of test cases. For each of the T cases, the first line will be the integer N, the number of nodes in the tree. The next line contains N &minus; 1 integers a<sub>1</sub>, a<sub>2</sub>, &middot; &middot; &middot; , a<sub>N&minus;1</sub> where a<sub>i</sub> is the downstream neighbour of the node with ID i. Then follows a line with N &minus; 1 integers w<sub>1</sub>, w<sub>2</sub>, &middot; &middot; &middot; , w<sub>N&minus;1</sub> where w<sub>i</sub> is the weight of the neural connection going out from the node with ID i. Note that the node with ID 0 will always be the output node.</p>

<ul>
	<li>0 &lt; T &le; 50</li>
	<li>0 &lt; N &le; 10000</li>
	<li>0 &lt; w<sub>i</sub> &le; 10</li>
...
<p>Output &ldquo;FREAK OUT&rdquo; (without the quotes) if the final value of the neural net&rsquo;s output node is even. Otherwise, output the final value of the output node, modulo 1,000,000,007.</p>