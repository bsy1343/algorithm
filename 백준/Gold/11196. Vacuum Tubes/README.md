...
제출: 66, 정답: 29, 맞힌 사람: 22, 정답 비율: 43.137%
...
<p>The first line of input contains three positive integers, L<sub>1</sub> and L<sub>2</sub>, denoting the distances explained above in mm (1 &le; L<sub>1</sub>, L<sub>2</sub> &le; 10 000) and N, the number of available tubes (4 &le; N &le; 2 000). The following N lines each contain a single positive integer less than or equal to 10 000, the length of a tube in mm.</p>

### 출력

<p>Output one line containing the maximum total length of air that can be avoided, i.e., the sum of the four tubes chosen. If there are no two pairs of tubes fitting into the setup, output the single word &ldquo;Impossible&rdquo; instead.</p>