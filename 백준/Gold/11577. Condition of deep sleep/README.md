...
제출: 1306, 정답: 450, 맞힌 사람: 340, 정답 비율: 44.041%
...
<p>입력의 첫 줄에는 특별한 전구의 개수 N(1 &le; N &le; 100,000)과 한 번의 버튼 조작으로 상태를 반전시킬 수 있는 전구의 개수 K(1 &le; K &le; N)가 주어진다.</p>

<p>다음 줄에는 N개의 S<sub>i</sub>가 공백을 기준으로 주어진다. S<sub>i</sub>는 i(1 &le; i &le; N)번째 전구의 상태를 나타내며 1은 전구가 켜져 있는 것을, 0은 전구가 꺼져있는 것을 의미한다.&nbsp;</p>

### 출력

<p>모든 전구를 끌 수 있는 최소한의 버튼의 조작 횟수를 출력한다. 한 번의 조작은 정확히 연속된 K개 전구의 상태를 반전시킬 수 있음에 유의한다. 만약 어떠한 조작으로도 모든 전구를 끄는 것이 불가능하다면 &ldquo;Insomnia&rdquo;(따옴표는 제외)를 출력한다.</p>