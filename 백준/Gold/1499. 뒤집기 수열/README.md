...
제출: 469, 정답: 54, 맞힌 사람: 40, 정답 비율: 16.807%

### 분류

다이나믹 프로그래밍, 자료 구조, 그래프 이론, 그래프 탐색, 너비 우선 탐색, 해시를 사용한 집합과 맵, 트리를 사용한 집합과 맵
...
<p>뒤집기 수열은 다음 조건을 만족해야 한다. 뒤집기 수열이 다음과 같이 길이가 m이고 수열의 각 원소가 첫 번째 부터 r(i<sub>1</sub>,j<sub>1</sub>), r(i<sub>2</sub>,j<sub>2</sub>), ... r(i<sub>m</sub>,j<sub>m</sub>) 이라면, i<sub>1</sub>&nbsp;&le; i<sub>2</sub>&nbsp;&le; ...&nbsp;&le; i<sub>m</sub>이고, j<sub>1</sub>&nbsp;&ge; j<sub>2</sub> &ge; ... &ge; j<sub>m</sub>인 조건을 만족해야 한다.</p>
