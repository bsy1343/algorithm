...
제출: 1124, 정답: 499, 맞힌 사람: 413, 정답 비율: 42.709%
...
<p>첫 줄에 게임 로그의 수 N이 주어진다. (1 &le; N &le; 32)</p>

<p>이어 N줄에 걸쳐, 돌을 둔 위치 R C가 주어진다. 이는 돌을 R행 C열에 놓았다는 의미이다. 행은 위부터 순서대로 1, 2, 3, &hellip; 으로 번호를 매기며, 열은 왼쪽부터 순서대로 1, 2, 3, ... 으로 번호를 매긴다. 둘 중 한 명이 돌을 둘 수 없어 턴을 넘기는 경우가 생기는 게임은 입력으로 주어지지 않는다.</p>
...
<p>1행 ~ 6행 : 행마다 6글자로, 6*6 격자 형태로 게임판의 최종 상태를 출력한다. 빈 칸은 &lsquo;.&rsquo; (ASCII 46), 검은 돌은 &lsquo;B&rsquo; (ASCII 66), 하얀 돌은 &lsquo;W&rsquo; (ASCII 87) 로 출력한다.</p>

<p>7행 : 검은 돌을 잡은 플레이어가 승리했다면 Black을, 아니라면 White를 출력한다. 비기는 경우가 없는 입력만 주어진다.</p>

### 힌트

<p>원래 정식 오델로는 8*8 보드판에서 진행하지만, 설명의 편의를 위해 6*6 보드판으로 변경하였다.</p>