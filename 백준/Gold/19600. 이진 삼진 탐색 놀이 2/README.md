...
제출: 208, 정답: 64, 맞힌 사람: 50, 정답 비율: 35.971%
...
<pre>
<code>binary_sea</code>rch(A[0..N-1], value, left, right) {
    mid = (left + right) / 2
    if (A[mid] == value)
        return mid
    else if (value &lt; A[mid])
...
<pre>
<code>ternary_sea</code>rch(A[0..N-1], value, left, right) {
...
<p>첫째 줄에 쿼리의 수 Q(1&nbsp;&le; Q&nbsp;&le; 500,000)가&nbsp;주어진다. 둘째 줄부터 Q+1줄까지 쿼리의 정보&nbsp;N S E가&nbsp;주어진다. N은 배열 A의 원소 개수, S는 구간의 시작, E는 구간의 끝을 나타낸다.</p>
