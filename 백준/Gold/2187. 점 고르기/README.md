# [Gold V] 점 고르기 - 2187

[문제 링크](https://www.acmicpc.net/problem/2187)

### 성능 요약

시간 제한: 2 초, 메모리 제한: 128 MB

### 통계

제출: 489, 정답: 248, 맞힌 사람: 207, 정답 비율: 50.860%

### 분류

브루트포스 알고리즘

### 문제 설명

<p>평면에 N(1 &le; N &le; 1,000)개의 점들이 있다. 각각의 점들은 정수 값으로 어떤 가중치 S(1 &le; S &le; 2,000,000)를 가지고 있다. 또 각각의 점들은 (r, c)의 좌표를 갖는데 이는 (행, 열) 순이다. 또한 1 &le; r &le; 2,000,000과 1 &le; c &le; 2,000,000가 성립한다.</p>

<p>이제 우리는 세로로 A(1 &le; A &le; 2,000,000), 가로로 B(1 &le; B &le; 2,000,000)만큼의 직사각형을 쳐서 이 중 몇 개의 점들을 이 사각형 안에 포함시키려고 한다. 이때, 사각형 안에 포함된 점들 중, &ldquo;S의 최대-S의 최소&rdquo;가 최대가 되도록 하려 한다.</p>

<p>예를 들어 다음과 같은 경우를 보자.</p>

<p style="text-align: center;"><img alt="" src="%EB%B0%B1%EC%A4%80/Gold/2187.%E2%80%85%EC%A0%90%E2%80%85%EA%B3%A0%EB%A5%B4%EA%B8%B0/2c9911bf.png" data-original-src="%EB%B0%B1%EC%A4%80/Gold/2187.%E2%80%85%EC%A0%90%E2%80%85%EA%B3%A0%EB%A5%B4%EA%B8%B0/2c9911bf.png" data-original-src="https://upload.acmicpc.net/d08784a4-3ad9-4376-9659-14bbe399e7df/-/preview/" onerror="this.onerror=null;this.src=this.dataset.originalSrc" style="width: 436px; height: 172px;" /></p>

<p>이는 예제 데이터와 같은 경우고, 회색 부분처럼 선택했을 때, 가중치 최대-가중치 최소가 7이 되고 이 경우가 이 값이 최대가 되는 경우이다.</p>

### 입력

<p>첫째 줄에 세 정수 N, A, B가 주어진다. 다음 N개의 줄에는 각 점의 r(세로좌표), c(가로좌표), S(가중치)를 나타내는 세 정수가 주어진다.&nbsp;두 점이 같은 위치를 공유하는 경우는 없다.</p>

### 출력

<p>첫째 줄에 가중치 최대 - 가중치 최소가 최대가 될 경우의 이 값을 출력한다.</p>