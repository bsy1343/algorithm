...
제출: 59, 정답: 30, 맞힌 사람: 24, 정답 비율: 55.814%
...
<p>The first line contains positive integers n, m and q - the number of boxes (also toys), the number of Martin&rsquo;s friends and the number of questions, respectively.</p>

<p>The k-th of the following m lines contains an array of positive integers p<sub>1</sub>, ..., p<sub>n</sub> that is used by Martin&rsquo;s k-th friend for putting the toys back into the boxes. Each positive integer from 1 to n appears exactly once in the array.</p>

<p>Each of the following q lines contains two positive integers a and b (1 &le; a, b &le; n) which represent a question.</p>
...
<p>In every subtask, it holds that 1 &le; n, m &le; 1000, 1 &le; q &le; 500 000.</p>

### 힌트

<p>Clarification of the first example:</p>

<p>For the first question, the toy with label 1 is already initially in the box with label 1 so the answer is immediately <code>DA</code>.</p>

<p>For the second question, notice that however many times Martin calls his friend over, the boxes with labels 1 and 2 never change their content, so the answer is <code>NE</code>.</p>

<p>For the third question, notice that after each meetup, the contents of boxes 3 and 4 get exchanged, so after only one meetup the toy with label 3 will find itself in box with label 4 and the answer is <code>DA</code>.</p>