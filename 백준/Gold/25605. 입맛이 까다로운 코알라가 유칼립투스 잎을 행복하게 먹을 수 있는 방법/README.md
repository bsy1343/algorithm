...
제출: 160, 정답: 40, 맞힌 사람: 39, 정답 비율: 34.821%
...
<p>항공 동물원에서 배식통에 넣어주는 유칼립투스 잎의 순서를 알고 있을 때, 이 프로그램을 통해 코알라가 $M$일 동안 유칼립투스 잎을 먹어서 얻을 수 있는 행복도의 최대치를 구하시오.</p>

### 입력

<p>첫째 줄에 유칼립투스 잎의 개수 $N$, 유칼립투스를 먹을 수 있는 기간인 $M$일이 주어진다. $(1 \le N \le 1\,000, 1 \le M \le 100)$</p>

<p>둘째 줄에는 코알라가 몸 안에 축적할 수 있는 독의 최대 한계 용량 $A$, 코알라가 하루에 해독할 수 있는 독의 양 $B$, 현재 코알라의 몸 안에 축적된 독의 양 $C$가 공백을 두고 주어진다. $(1 \le A \le 100, 0 \le B, C \le A)$</p>

<p>셋째 줄부터 $N$개의 줄에 배식통에 순서대로 넣어주는 유칼립투스 잎에 대한 정보가 주어지는데, 해당 유칼립투스 잎을 먹을 때 코알라 몸 안에 축적되는 독의 양 $x$와 얻을 수 있는 행복도 $y$가 공백을 사이에 두고 주어진다. $(1 \le x \le A, 1 \le y \le 1\,000)$</p>

<p>입력으로 주어지는 모든 값은 정수이다.</p>

### 출력

<p>$M$일동안 코알라가 얻을 수 있는 행복도의 최대치를 출력한다.</p>