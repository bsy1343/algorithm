...
제출: 527, 정답: 149, 맞힌 사람: 117, 정답 비율: 31.536%
...
<p>&ldquo;해치웠나?&rdquo;</p>

<p>악의 단체의 리더 한별이는 자신의 부하들이 &ldquo;해치웠나?&rdquo;라는 말을 들을 때마다 강해지는 것을 보고 이를 악용해 부하들을 강화시켜 세계를 정복하려고 한다.</p>

<p>부하들은 좌우 방향의 직선 위에 $1$의 간격을 두고 서 있고, 왼쪽으로부터 $i$번째에 있는 부하는 현재 강함 수치 $p_i$를 가지고 있으며 강함 수치 $t_i$를 목표로 하고 있다.</p>

<p>한별이는 부하들을 강화시키기 위해 어떤 지점에서 오른쪽을 보고 &ldquo;해치웠나?&rdquo;라는 대사를 말하는데, 이 때문에 한별이가 대사를 말하는 지점과 그 오른쪽에 있는 부하들은 한별이의 목소리를 듣고 강해진다. 그러나, 거리가 멀어질수록 한별이의 대사를 듣기 어렵기 때문에, 한별이의 목소리의 크기가 $K$일 때, 한별이가 서 있는 지점에서 오른쪽으로 $i$ ($i \ge 0$) 칸 떨어진 지점의 부하는 $\max(0,K-i)$ 만큼 강해진다.</p>
...
<ul>
	...
	<li>$1 \le K \le 500\,000$</li>
	<li>$0 \le p_i \le 10^9$</li>
	<li>$0 \le t_i \le 10^9$</li>
</ul>