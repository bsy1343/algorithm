...
제출: 359, 정답: 94, 맞힌 사람: 78, 정답 비율: 28.261%
...
<p>파댕이를 제외한 나머지 댕댕이들은 그저 그 자리를 지키며 누군가 자신을 데려가기만 바라고 있어 폭탄주를 마실까 봐 무서운 파댕이는 직접 자신의 무리를 이끌어 $M$마리를 만들려 한다. 파댕이가 현재 $P$번째 구역에 있으면 $1$초마다 $P-1$번째 또는 $P+1$번째 구역으로 이동할 수 있고, 그 구역에 있는 무리를 데려오거나 지나갈 수 있다. 다른 무리를 데려올 때는 시간 소요는 들지 않으며, 게임을 시작한 지 정확히 $T$초가 지났을 때도 다른 무리를 데려올 수 있다.</p>

<p>파댕이가 제한 시간 $T$초 안에 무사히 $M$마리 댕댕이들을 모을 수 있는지 알아보자!</p>

### 입력

<p>첫 번째 줄에 일렬로 나열된 구역의 수 $N$, 사회자가 정한 마릿수 $M$, 게임 제한 시간 $T$를 의미하는 정수가 공백으로 구분되어 주어진다. $(1 \le N \le 100, 1 \le M \le 100,000, 1 \le T \le 100)$</p>

<p>두 번째 줄에는 $1$번째 구역부터 $N$번째 구역까지 각 구역에 있는 무리의 댕댕이 수를 나타내는 정수 $Q_{1}, Q_{2}, \cdots, Q_{N}$이 공백으로 구분되어 주어진다. $(1 \le Q_{i} \le 1,000)$</p>

<p>세 번째 줄에는 시작전 파댕이가 속해 있는 무리의 위치 $P$가 정수로 주어진다. $(1 \le P \le N)$</p>
