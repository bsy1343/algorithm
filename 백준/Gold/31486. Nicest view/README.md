...
제출: 64, 정답: 45, 맞힌 사람: 43, 정답 비율: 69.355%
...
<p>Paris is so crowded with tourists during the Olympic games! You want to escape the city and go on a hike on a linear trail path. Every kilometre on that trail, including at start and end, is a milestone, on which is written the stone&rsquo;s altitude. The slope between two consecutive stones is constant, and no two stones have the same altitude.</p>
...
<ul>
	...
	<li>$0 \le H_k \le 1\, 000\, 000$ for all $k \le N$</li>
	<li>the integers $H_k$ are pairwise distinct.</li>
</ul>