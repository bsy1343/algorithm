...
제출: 42, 정답: 30, 맞힌 사람: 27, 정답 비율: 71.053%
...
<p><sup>2</sup>If you haven&rsquo;t done so, check out Parkside Restaurant and Galen Athletic Center.</p>
...
<p>The first line of each data set contains numbers h and w, the height and width of the campus map (1 &le; h, w &le; 30). This is followed by h lines of w characters each. Each character is one of the following:</p>
...
<p>For each data set, first output &ldquo;Data Set x:&rdquo; on a line by itself, where x is its number. Then, output the total length traveled by you and all your friends together for the best pair of meeting point and restaurant. (You do not need to output the sequence.)</p>

<p>If there is no meeting point, or no restaurant, accessible from all starting points, then output &ldquo;Impossible&rdquo; instead of the total length.</p>