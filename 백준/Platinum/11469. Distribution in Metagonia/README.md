...
제출: 292, 정답: 135, 맞힌 사람: 117, 정답 비율: 55.981%

### 분류

수학, 그리디 알고리즘, 애드 혹, 정수론, 해 구성하기
...
<p>The first line of input file contains a single integer t &mdash; the number of years to come (1 &le; t &le; 1000). Each of the following t lines contains a single integer n<sub>i</sub> &mdash; the number of cubes to distribute in i-th year (1 &le; n<sub>i</sub> &le; 10<sup>18</sup>).</p>

### 출력

<p>For each year i output two lines. The first line should contain m<sub>i</sub> &mdash; the number of families that would receive at least one cube in i-th year (1 &le; m<sub>i</sub> &le; 100). The second line should contain m<sub>i</sub> integers &mdash; the number of cubes received by each family. The sum of these numbers should be equal to n<sub>i</sub>.</p>