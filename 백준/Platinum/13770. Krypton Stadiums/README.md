...
제출: 24, 정답: 8, 맞힌 사람: 7, 정답 비율: 43.750%
...
<p>The planet of Krypton contains n cities. These cities are located in distinct locations all along one straight line running west-east. The cities are labelled 0, 1, 2, . . . , n &minus; 1 in order from west to east. Each city is home to one team and one stadium. Each stadium will have two corresponding integers (a<sub>i</sub> and b<sub>i</sub>) defining an interval of cities that may play at that stadium. That is, a team from city x may only play at stadium i if a<sub>i</sub> &le; x &le; b<sub>i </sub>. It is guaranteed that every team will be able to play at its home stadium (a<sub>i </sub>&le; i &le; b<sub>i</sub>).</p>
