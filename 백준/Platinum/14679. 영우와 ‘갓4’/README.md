...
제출: 371, 정답: 94, 맞힌 사람: 53, 정답 비율: 26.633%

### 분류

수학, 구현, 시뮬레이션, 분할 정복을 이용한 거듭제곱

### 문제 설명

<p>게임을 좋아하는 영우는 오늘도 &lsquo;갓4&rsquo; 라는 게임을 한다. &lsquo;갓4&rsquo;의 게임 시스템은 신기한데 우선 캐릭터와 몬스터는 각각 공격력(A), 방어력(D), 최대 체력(H)이 주어진다. &lsquo;갓4&rsquo;의 전투 시스템은 1:1 전투이며 다음과 같다.</p>

<ol>
	<li>자기의 턴이 오면 상대방을 공격할 수 있으며, [(자신의 공격력 &ndash; 상대방의 방어력), 1] 중에 더 높은값만큼 상대방의 체력을 깎는다.</li>
...
<p>프로그램의 입력은 표준 입력으로 받는다. 입력의 첫 줄에는 몬스터의 수 N이 주어진다. (1 &le; N &le; 50,000,000) 두 번째 줄에는 영우가 키우는 캐릭터의 공격력(A), 방어력(D), 최대 체력(H)이 차례대로 주어진다. (1 &le; A &le; 100) (1 &le; D &le; 3) (1 &le; H &le; 1000) 세 번째 줄에는 첫 번째 몬스터의 공격력(MA), 방어력(MD), 최대 체력(MH)이 차례대로 주어진다. (1 &le; MA &le; 100) (1 &le; MD &le; 3) (1 &le; MH &le; 1000) 넷째 줄에 몬스터의 능력치를 구하는데 사용되는 값(A<sub>p</sub>, A<sub>a</sub>, D<sub>p</sub>, D<sub>a</sub>, H<sub>p</sub>, H<sub>a</sub>)이 차례대로 주어진다. (1 &le; A<sub>p</sub>, D<sub>p</sub>, H<sub>p</sub> &le; 1,000,000,000) (0 &le; A<sub>a</sub> &le; 99) (0 &le; D<sub>a</sub> &le; 2) (0 &le; H<sub>a</sub> &le; 999)&nbsp;</p>
