...
제출: 603, 정답: 129, 맞힌 사람: 104, 정답 비율: 24.703%

### 분류

자료 구조, 세그먼트 트리, 느리게 갱신되는 세그먼트 트리, 덱
...
<p>소각 작업은 L번 칸부터 R번 칸까지(L &le; R) 연속된 칸에 있는 쓰레기&nbsp;모두를 한 번에 태우는 식으로 이루어진다. 이렇게 한 번 태우고 나면 L번 칸부터 R번 칸까지는 비게 되는데, 그때 대기열의 앞에서부터 순서대로 쓰레기를 가져와 L번 칸부터 R번 칸까지 채운다. 쓰레기를 가져와 채우다가 더 이상 대기열 상에 쓰레기가 남지 않았으면 그 뒤로는 아무것도 놓지 않는다.</p>
...
<p>첫 번째 줄에 N, M, K, Q의 값들이 순서대로 입력된다. (1 &le; N, M, K, Q &le; 5&times;10<sup>5</sup>)</p>

<p>두 번째&nbsp;줄에 N개의 A<sub>i</sub> 값들이 주어진다. (1 &le; A<sub>i</sub> &le; K)</p>

<p>Q개 줄에 명령들이 주어진다. 명령의 의미를 나타내는 수 o가 먼저 주어진다. (1 &le; o &le; 4)</p>

<ul>
	<li>o=1 이면 첫 번째 명령을 뜻하며, L과 R이 주어진다. (1 &le; L &le; R &le; M)</li>
	<li>o=2 이면 두 번째 명령을 뜻하며, i가 주어진다. (1 &le; i &le; M)</li>
	<li>o=3 이면 세 번째 명령을 뜻하며, p와 q가 주어진다. (1 &le; p &le; K, 1 &le; q &le; 10<sup>6</sup>)</li>
	<li>o=4 이면 네 번째 명령을 뜻하며, t가 주어진다. (1 &le; t &le; 현재 대기열 상 쓰레기의 개수)</li>
</ul>
