...
제출: 249, 정답: 82, 맞힌 사람: 71, 정답 비율: 36.041%
...
<p>어느 날 택희는 원주를 <em>N</em>(&ge;<em>M</em>)등분하는 점을 찍고, 각 지점에 시계방향으로 1, 2, 3, &hellip;, <em>N</em>의 번호를 붙였다. 각 지점의 번호를 시계방향으로 읽으면 1, 2, 3, &hellip;, <em>N</em>이 되며, <em>N</em>번 지점 다음에는 1번 지점이 있게 될 것이다. 그 후, 모든 개미가 서로 다른 어떤 지점 위에 있도록 개미들을 놓았다. 개미들은 시계방향 또는 반시계방향의 초기 방향을 바라보고 있으며, 택희가 &ldquo;시작&rdquo;을 외치면 모든 개미들은 바라보고 있는 방향으로 이동하게 된다. 시작 시각은 0이며, 그 이후 정확히 1초마다 개미들은 다음 지점에 도착한다.</p>
...
<p>첫 줄에 원주 위의 점의 개수 <em>N</em> (2&nbsp;&le; <em>N</em> &le; 10<sup>5</sup>)과 개미의 수 <em>M</em> (1 &le; <em>M</em> &le; <em>N</em>), 택희가 궁금해하는 정보의 수 <em>Q</em> (1 &le; <em>Q</em> &le; 10<sup>5</sup>)가 주어진다.</p>

<p>이어 <em>M</em>개의 줄에 각 개미들의 초기 위치 <em>P<sub>i</sub></em>와 진행방향 <em>d<sub>i</sub></em>가 주어진다. (1 &le; <em>P<sub>i</sub></em> &le; <em>N</em>, <em>d<sub>i</sub></em> = 0 or 1)</p>

<p>모든 <em>P<sub>i</sub></em>는 서로 다르다.</p>

<p><em>d<sub>i&nbsp;</sub></em>= 0일 경우 개미는 시계방향으로 이동하고, <em>d<sub>i&nbsp;</sub></em>= 1일 경우 반시계방향으로 이동한다.</p>

<p>이어 <em>Q</em>줄에 걸쳐, 택희가 궁금해하는 정보 <em>P</em> <em>X</em>가 주어진다. (1 &le; <em>P</em> &le; <em>N</em>, 1 &le; <em>X</em> &le; 10<sup>9</sup>)</p>
