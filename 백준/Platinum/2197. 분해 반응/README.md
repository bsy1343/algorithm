...
제출: 469, 정답: 215, 맞힌 사람: 166, 정답 비율: 48.538%
...
<p>N(1 &le; N &le; 150)개의 원자로 구성된 분자가 있다. 각각의 원자들은 1부터 N까지의 번호가 붙어 있으며, 모든 원자들이 하나의 분자로 모두 연결되어 있다. 화학 실험을 할 때 결합 반응과 분해 반응에 대한 실험을 쉽게 하기 위해서, 원자들을 총 N-1 개의 결합만을 하고 있다.</p>
...
<p>첫째 줄에 두 정수 N, M(1 &le; M &le; N)이 주어진다. 다음 N-1개의 줄에는 원자들의 연결 상태를 나타내는 서로 다른 두 정수 A, B(1 &le; A, B &le; N)가 주어진다. 이는 A번 원자와 B번 원자가 결합을 이루고 있다는 의미이다.</p>
