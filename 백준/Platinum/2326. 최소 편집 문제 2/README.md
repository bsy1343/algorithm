...
제출: 324, 정답: 51, 맞힌 사람: 41, 정답 비율: 24.551%
...
<p>최소 편집 문제는 두 개의 문자열이 주어졌을 때 &ldquo;삽입(insert)&rdquo;, &ldquo;삭제(delete)&rdquo;, &ldquo;대체(replace)&rdquo;의 연산으로 하나의 문자열을 다른 하나의 문자열과 같게 만드는데 사용되는 최소 편집 회수를 구하는 문제이다. 하지만 이렇게 익히 알려진 문제를 또 다시 낼 리가 있겠는가? 그래서 문제에 새로운 편집기술이 하나가 추가되었는데 기존의 편집기술도 포함해서 다시 문제를 정리 해보도록 하자.</p>
