...
제출: 21, 정답: 12, 맞힌 사람: 11, 정답 비율: 64.706%

### 분류

구현, 그래프 이론, 기하학, 최단 경로, 선분 교차 판정
...
<p>入力の 1 行目には警備員の数を表す整数 a (1 &le; a &le; 10) と建 物の数を表す整数 b (1 &le; b &le; 50) と不審物の数を表す整数 c (1 &le; c &le; 10) が書かれている．</p>

<p>2 行目には Y センターの敷地の大きさを表す 2 つの整数 w, h (1 &le; w, h &le; 1000) が横，縦の 順に書かれている．</p>

<p>i + 2 行目 (1 &le; i &le; a) には，i 人目の警備員の場所を表す 2 つの整数 x<sub>i</sub>, y<sub>i</sub> (0 &le; x<sub>i</sub> &le; w, 0 &le; y<sub>i</sub> &le; h) が書かれている．j + a + 2 行目 (1 &le; j &le; b) には，j 人目の建物の場所を表す 4 つの整数 s<sub>j</sub>, t<sub>j</sub>, u<sub>j</sub>, v<sub>j</sub> (0 &le; s<sub>j</sub> &lt; u<sub>j</sub> &le; w, 0 &le; t<sub>j</sub> &lt; v<sub>j</sub> &le; h) が書かれている．k + a + b + 2 行目 (1 &le; k &le; c) には，k 人目の不審物の数を表す 2 つの整数 p<sub>k</sub>, q<sub>k</sub> (0 &le; p<sub>a</sub> &le; w, 0 &le; q<sub>a</sub> &le; h) が書かれている．</p>
