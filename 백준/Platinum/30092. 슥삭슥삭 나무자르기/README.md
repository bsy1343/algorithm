...
제출: 303, 정답: 142, 맞힌 사람: 96, 정답 비율: 43.836%
...
<ul>
	<li>$a$ $b$ $c$ $d$: 정점 $a$에서 $b$로 가는 최단 경로에 속한 모든 간선을 제거하였을 때, $c$에서 $d$로 가는 경로가 존재한다면 &ldquo;<code>YES</code>&rdquo;를 존재하지 않는다면 &ldquo;<code>NO</code>&rdquo;를 출력한다. 따옴표는 출력하지 않는다.</li>
</ul>
