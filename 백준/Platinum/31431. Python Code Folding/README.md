...
제출: 33, 정답: 14, 맞힌 사람: 13, 정답 비율: 44.828%
...
<ul>
	<li><strong>라인 (Line)</strong>은 Python 코드의 한 행을 나타낸다. 이 문제에서, 각 라인은 &lsquo;헤더&rsquo; 또는 &lsquo;심플문&rsquo; 중 하나이다.</li>
...
<ul>
	<li><code><span style="color:#e74c3c;">t</span></code> $x$ &mdash; <strong>접히지 않은 전체 코드</strong>에서 $x$번째 블록이 펴져 있으면 접고, 접혀 있으면 편다. 상위 블록이 접혀있어 $x$번째 블록이 보이지 않는 경우는 주어지지 않는다. $(1 \leq x \leq $ 헤더 라인의 총 개수$)$</li>
	<li><code><span style="color:#e74c3c;">p</span></code> &mdash; 현재 보이는 코드의 라인 개수를 출력한다. 이 쿼리는 적어도 하나 이상 주어진다.</li>
</ul>
