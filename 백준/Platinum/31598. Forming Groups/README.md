...
제출: 201, 정답: 64, 맞힌 사람: 59, 정답 비율: 33.333%
...
<p>Formally, for each $j$ ($1 &le; j &le; k$) and for each $i$ ($0 &le; i &lt; n/k$), the $(i \times k +j)$-th student from the left will be assigned to group $j$. It can be shown that each student will be assigned to exactly one group and all the groups have the same number of students.</p>
...
<p>The first line of input contains one integer $t$ ($1 &le; t &le; 100\, 000$) representing the number of test cases. After that, $t$ test cases follow. Each of them is presented as follows.</p>

<p>The first line of a test case contains two integers $n$ and $a_1$ ($2 &le; n &le; 10^6$; $1 &le; a_1 &le; 1000$). The next line contains $n - 1$ integers $a_2, a_3, \dots , a_n$ ($1 &le; a_i &le; 1000$ for all $i$).</p>
