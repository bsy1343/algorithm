...
제출: 181, 정답: 54, 맞힌 사람: 41, 정답 비율: 28.873%

### 분류

다이나믹 프로그래밍, 자료 구조, 세그먼트 트리, 이분 탐색, 매개 변수 탐색
...
<p>$x=2$일 때, 왼쪽에서 5번째 옷과 9번째 옷을 제거하면 옷장은 <strong>거의 아름다운 상태</strong>가 된다. 이보다 더 작은 $x$로는 옷장을 <strong>거의 아름다운 상태</strong>로 만들 수 없다.</p>