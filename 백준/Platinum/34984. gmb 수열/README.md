...
제출: 89, 정답: 49, 맞힌 사람: 46, 정답 비율: 53.488%
...
<p>길이가 $N$인 수열 $B=[B_1, B_2, \cdots, B_N]$가 주어졌을 때, $B$의 부분 수열(subsequence)은 다음과 같이 정의한다.</p>

<ul>
<li>$[B_{i_1}, B_{i_2}, \cdots, B_{i_m}]\ \ (1\le i_1&lt;i_2&lt;\cdots&lt;i_m\le N;\ 0\le m\le N)$</li>
</ul>

<p>이때, 두 부분 수열이 동일한 원소를 같은 순서로 포함하더라도, 인덱스 선택 $(i_1, i_2, \cdots, i_m)$이 서로 다르면 서로 다른 부분 수열로 본다. 즉 부분 수열은 값의 나열뿐 아니라 인덱스 선택의 조합으로 구별된다.</p>