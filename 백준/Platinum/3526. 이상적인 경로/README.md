...
제출: 660, 정답: 154, 맞힌 사람: 81, 정답 비율: 17.345%

### 분류

그래프 이론, 그리디 알고리즘, 그래프 탐색, 너비 우선 탐색, 최단 경로, 역추적
...
<p>첫째 줄에 방의 수 n과 복도의 수 m이 주어진다. (2 &le; n &le; 100,000, 1 &le; m &le; 200,000) 다음 m개 줄에는 복도의 정보가 주어지며, 정보는 세 정수 a<sub>i</sub>, b<sub>i</sub>, c<sub>i</sub>로 이루어져 있다. a<sub>i</sub>와 b<sub>i</sub>는 복도가 연결하는 방의 번호이고, c<sub>i</sub>는 복도의 색상이다. (1 &le; a<sub>i</sub>, b<sub>i</sub> &le; n, 1 &le; c<sub>i</sub> &le; 10<sup>9</sup>) 복도는 양방향이며, 두 방을 연결하는 복도의 개수는 1개보다 많을 수도 있다. 또, 자기 자신으로 되돌아오는 복도가 존재할 수도 있다. 항상 1번 방에서 n번 방으로 갈 수 있는 미로만 입력으로 주어진다.</p>
