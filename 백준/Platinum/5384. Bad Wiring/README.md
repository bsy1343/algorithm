...
제출: 51, 정답: 25, 맞힌 사람: 20, 정답 비율: 43.478%
...
<p><sup>2</sup>This means that S<sub>1</sub> turns on/off all the lights L<sub>1</sub> . . . L<sub>D+1</sub> and S<sub>n</sub> turns on/off all the lights L<sub>n-D</sub> . . . L<sub>n</sub>. Of course, if D &ge; n, then L<sub>D+1</sub> and L<sub>n-D</sub> will not exist either.</p>
...
<ul>
	<li>One line with two integers n (1 &le; n &le; 100) and D (0 &le; D &le; 15): the number of lights and the parameter mentioned above.</li>
...
<p>For every test case in the input, the output should contain one integer on a single line: the minimum number of times Ryu needs to flip a switch to turn off all the lights. If it is impossible to turn off all the lights, then output the string &ldquo;impossible&rdquo; instead.</p>

### 힌트

<p>In the first example below, flipping switch S<sub>4</sub> followed by S<sub>7</sub> will turn off all the lights.</p>