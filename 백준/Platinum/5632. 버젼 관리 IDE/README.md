...
제출: 294, 정답: 114, 맞힌 사람: 91, 정답 비율: 47.150%
...
<ul>
	<li>1 p s: 위치 p의 이후에 문자열 s를 삽입한다. (0 &le; p &le; L[vnow]) p가 0인 경우에는 버퍼의 가장 앞에 문자열을 추가하는 것이다. s는 1~100글자로 이루어진 문자열이다.</li>
	<li>2 p c: 위치 p부터 c개의 문자를 제거한다. (p &ge; 1, p+c &le; L[vnow]+1)</li>
	<li>3 v p c: 버젼 v에서 위치 p부터 c개의 문자를 출력한다. (p &ge;&nbsp;1, p+c &le; L[v]+1)</li>
</ul>
...
<p>첫째 줄에 명령의 수 T (1 &le; T &le; 50,000)가 주어진다. 다음 T개의 줄에는 명령이 주어진다. 삽입된 문자열의 총 길이는 1,000,000을 넘지 않는다.</p>
