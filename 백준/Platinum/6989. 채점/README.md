...
제출: 1870, 정답: 509, 맞힌 사람: 303, 정답 비율: 27.697%
...
<ol>
	...
	<li>1번 문항이 맞으면 이 문항의 점수는 S<sub>1</sub>이다. i(2 &le; i &le; N)번 문항의 답이 맞을 경우에는 (i-1)번 문항의 정답 여부에 따라 이 문항의 점수가 다르게 정해진다.
...
<p>어떤 답안지에서 문항 1번부터 9번까지에 대하여 제출한 답이 맞는지(○) 혹은 틀린지(&times;)가 아래 &lt;표 2&gt;와 같다고 하자.</p>
...
<p>첫째 줄에 전체 문항 수 N(1 &le; N &le; 150)이 주어진다. 다음 줄에는 N개 문항들의 배점이 빈칸을 사이에 두고 차례대로 주어진다. 문항별 배점은 1 이상 100 이하의 정수이다. 마지막 줄에 자연수 K(1 &le; K &le; 2,000,000,000)가 주어진다.</p>

### 출력

<p>답안지의 총점으로 나올 수 없는 가장 작은 정수 M(M &ge; K)을 첫째 줄에 출력한다.</p>