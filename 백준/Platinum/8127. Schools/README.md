...
제출: 79, 정답: 52, 맞힌 사람: 46, 정답 비율: 63.014%
...
<p>There are n schools in Byteotia, and each has its number m, 1 &le; m &le; n, just as every school tends to. However, the previous king of Byteotia paid little attention to the school numbering, and generously allowed every new school to choose any number from the range 1&hellip;n. There is thus no guarantee that the numbers are unique, in fact it seems highly unlikely. Clearly some schools may have chosen the same number, and as a consequence some numbers from the range 1&hellip;n might be not used at all. A perfect numbering would be a permutation, i.e. a one that assigns each number from the specified range to exactly one school.</p>
...
<p>The first line of the standard input contains one integer n (1 &le; n &le; 200), denoting the number of schools in Byteotia. In the following n lines there are descriptions of the schools themselves. The line no. i+1 (1 &le; i &le; n) contains four integers m<sub>i,</sub> a<sub>i</sub>, b<sub>i</sub> and ki (1 &le; a<sub>i</sub> &le; m<sub>i</sub> &le; b<sub>i</sub> &le; n, 1 &le; k<sub>i</sub> &le; 1,000), separated by single spaces. These denote respectively: the current number of the i&rsquo;th school, the left and right endpoint of the i&rsquo;th school&#39;s tolerance interval regarding the change of number (it is a closed interval, therefore the new i&rsquo;th school&#39;s number m<sub>i</sub>&rsquo; must satisfy the inequality a<sub>i</sub> &le; m<sub>i</sub>&rsquo; &le; b<sub>i</sub>) and the cost of changing the number of i&rsquo;th school by 1.</p>
