...
제출: 83, 정답: 25, 맞힌 사람: 18, 정답 비율: 35.294%
...
<p>Input contains several tests. The first line contains one integer t (1 &le; t &le; 100) &mdash; the number of tests. The tests follow.</p>

<p>Each test has the following structure. The first line of the test contains two integers: n and h (1 &le; n &le; 10<sup>5</sup>, 1 &le; h &le; 10<sup>9</sup>) &mdash; the number of cars and the length of the tunnel. The second line contains n integers a<sub>i</sub> (1 &le; a<sub>i</sub> &le; 10<sup>9</sup>) &mdash; the lengths of the cars. The third line contains n integers, the i-th of them is 1 if the i-th car initially has the light on, or 0 if it has the light off. Cars are described in order they would enter the tunnel.</p>

<p>The sum of values of n for all tests doesn&#39;t exceed 10<sup>6</sup>.</p>

### 출력

<p>For each test case output one integer &mdash; the minimal number of cars that Peter must turn the light on so that there were no dark moment.</p>