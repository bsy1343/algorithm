...
제출: 110, 정답: 75, 맞힌 사람: 63, 정답 비율: 67.021%
...
<p>The FBI has recently changed its Universal Control Numbers (UCN) for identifying individuals who are in the FBI&rsquo;s fingerprint database to an eight digit base 27 value with a ninth check digit. The digits used are:</p>
...
<p>For each data set there is one line of output. The single output line consists of the data set number, K, followed by a single space followed by the string &ldquo;Invalid&rdquo; (without the quotes) or the decimal value corresponding to the first eight digits.</p>