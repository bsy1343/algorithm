...
제출: 3130, 정답: 808, 맞힌 사람: 640, 정답 비율: 25.529%
...
<p>첫 번째 줄에는 보도블록의 세로, 가로 R, C(1 &le; R, C &le; 1,000)크기가 주어진다. 다음 R개의 줄에는 C개의 문자로 이루어진 보도블록의 초기 상태가 주어진다. (가로 블록은 0로 표시되고, 세로 블록은 1로 표시된다.) 그 다음 줄에 이동 가능한 규칙의 개수 N(0 &le; N &le; 10)이 주어지고 그 다음 N개의 줄에 걸쳐 규칙 r(-R &le; r &le; R), c(-C &le; c &le; C)가 주어진다. 이 뜻은 현재 위치가 (0, 0) 일 때 (0+r, 0+c)로 이동 할 수 있음을 의미한다. ((0,0)왼쪽 위, (R &ndash; 1, C &ndash; 1)오른쪽 아래 블록이다.)</p>
