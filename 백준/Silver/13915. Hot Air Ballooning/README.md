...
제출: 721, 정답: 380, 맞힌 사람: 326, 정답 비율: 55.068%
...
<p>There are exactly nine types of balloons in Dave&rsquo;s school, and no trainee has flown more than nine times in a balloon, so Dave expresses each trainee list as an integer consisting of digits 1, 2, . . . , 9 and smaller than one billion. He thinks that this representation will help him to process the lists programmatically by a computer.</p>
...
<p>There are more test cases. Each case starts with a line containing one integer N (1 &le; N &le; 1 000) representing the number of trainees. Next, there are N lines, each line contains one integer representing the list of flights of one particular trainee.</p>
