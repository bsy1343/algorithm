...
제출: 47, 정답: 33, 맞힌 사람: 29, 정답 비율: 74.359%

### 분류

수학, 자료 구조, 정렬, 집합과 맵, 조합론, 해시를 사용한 집합과 맵, 확률론
...
<p>There are more test cases. Each case starts with a line containing one integer N (1 &le; N &le; 10<sup>5</sup>) specifying the number of towns on the map. Next, there are N lines each of which contains two integers x and y separated by space and specifying the coordinates of one town on the map. The coordinates are standard Cartesian coordinates in the plane. The absolute value of any coordinate does not exceed 10<sup>9</sup>.</p>

### 출력

<p>For each test case, print a single line with one floating point number P denoting the probability that there is a winner in the game. P should be printed with the maximum allowed error of 10<sup>&minus;6</sup>.</p>