...
제출: 219, 정답: 185, 맞힌 사람: 156, 정답 비율: 86.667%

### 분류

구현, 자료 구조, 문자열, 집합과 맵, 해시를 사용한 집합과 맵, 트리를 사용한 집합과 맵, 파싱
...
<p>The first line contains an integer N (1 &le; N &le; 1000) representing the number of email addresses in the database. Each of the next N lines contains a string of at most 100 characters representing an email address in the database. Each email address has the form localpart@provider where localpart is a non-empty list of labels with a &ldquo;<code>.</code>&rdquo; (dot) or a &ldquo;<code>+</code>&rdquo; (plus sign) between each pair of consecutive labels, and provider is a non-empty list of labels always with a &ldquo;.&rdquo; (dot) between each pair of consecutive labels. A label is a non-empty sequence of lowercase letters and/or digits. The character &ldquo;<code>+</code>&rdquo; (plus sign) appears at most once in each email address.</p>
