...
제출: 3723, 정답: 1892, 맞힌 사람: 1384, 정답 비율: 50.437%
...
<p>X를 45&deg; 의 배수만큼 시계방향 혹은 반시계방향으로 돌리려고 한다. X를 시계 방향으로 45&deg; 돌리면 아래와 같은 연산이 동시에 X에 적용되어야 한다:</p>

<ul>
	<li>X의 주 대각선을 ((1,1), (2,2), &hellip;, (n, n)) 가운데 열 ((n+1)/2 번째 열)로 옮긴다.</li>
	<li>X의 가운데 열을 X의 부 대각선으로 ((n, 1), (n-1, 2), &hellip;, (1, n)) 옮긴다.&nbsp;</li>
...
<p>반시계 방향으로 45&deg; 돌리는 경우도 위와 비슷하게 정의된다.</p>

<p>예를 들어, 아래 그림 중앙에 5x5 배열 X가 있고, 이 배열을 시계방향 혹은 반시계방향으로 45&deg; 돌렸을 때의 결과가 우측 그리고 좌측에 있다. 굵은 원소는 주 대각선 / 중간 열 / 부 대각선 / 중간 행에 위치한 원소이다.</p>
...
<table class="table table-bordered" style="width: 100%;">
	<tbody>
		...
		<tr>
			<td style="width: 33%; text-align: center;">X를 반시계 방향으로 45&deg; 회전한 경우</td>
			<td style="width: 34%; text-align: center;">배열 X (5x5)</td>
			<td style="width: 33%; text-align: center;">X를 시계 방향으로 45&deg; 회전한 경우</td>
		</tr>
	</tbody>
</table>
...
<p>각 테스트 케이스에 대해: 첫 줄에 배열의 크기를 나타내는 n (1 &le; n &lt; 500, n은 홀수) 그리고 각도 d가 주어진다. d는 0 &le; |d| &le; 360 을 만족하며 |d| 는 45의 배수이다. d가 양수이면 시계방향으로 d&deg; 돌려야 하고, 음수이면 반시계방향으로 |d|&deg; 돌려야 한다. 다음 n줄에 걸쳐 각 줄에 n개의 정수가 공백으로 구분되어 주어진다 (X의 원소들을 나타낸다). 각 값은 1 이상 1,000,000 이하의 정수이다.</p>
