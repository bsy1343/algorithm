...
제출: 16861, 정답: 5750, 맞힌 사람: 4911, 정답 비율: 34.080%
...
<p>하지만 피보나치 수 F(n)을 n이 음수인 경우로도 확장시킬 수 있다. 위의 식에서 n &gt; 1인 경우에만 성립하는 F(n) = F(n-1) + F(n-2)를 n &le; 1일 때도 성립되도록 정의하는 것이다. 예를 들어 n = 1일 때 F(1) = F(0) + F(-1)이 성립되어야 하므로, F(-1)은 1이 되어야 한다.</p>
