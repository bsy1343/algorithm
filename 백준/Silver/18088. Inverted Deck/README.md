...
제출: 201, 정답: 113, 맞힌 사람: 97, 정답 비율: 55.429%
...
<ul>
	<li>One line containing an integer n (1 &le; n &le; 10<sup>6</sup>), the number of cards in your collection.</li>
	<li>One line containing n integers v<sub>1</sub>, . . . , v<sub>n</sub> (1 &le; v<sub>i</sub> &le; 10<sup>9</sup> for all i), the current order of the cards&rsquo; rarity values.</li>
</ul>

### 출력

<p>If the cards can be sorted by reversing exactly one contiguous subsequence of the list, then output the 1-based start and end indices of such a subsequence. Otherwise, output &ldquo;impossible&rdquo;. If there are multiple valid solutions you may output any one of them.</p>