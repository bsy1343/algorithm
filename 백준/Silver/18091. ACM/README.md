...
<p>The first line contains integers N (1 &le; N &le; 1000) and M (1 &le; M &le; 15) from the task description.</p>
...
<ul>
	<li><code>S</code> represents the status of the task &ndash; &lsquo;<code>+</code>&rsquo; means the task is solved correctly, &lsquo;<code>-</code>&rsquo; means it is solved incorrectly and &lsquo;<code>?</code>&rsquo; means that the last submission was sent when the standings were already frozen.</li>
	...
	<li><code>V</code> represents the time when the last submission was sent by that team for this specific task. It is given in the format <code>HH:MM:SS</code> (with leading zeroes) and is less than 5 hours. The whole <code>/V</code> part is omitted if the task is not solved correctly (status &rsquo;<code>-</code>&rsquo;)</li>
</ul>
...
<p>Clarification of the fist example: Nothing will change after the standings our unfrozen. Therefore, our heroes will remain in the first place!</p>

<p>Clarification of the second example: In the worst case our heroes will only lose from the team <code>StoJeZivot</code>, thereby finishing in second place.</p>

<p>Clarification of the third example: In the worst case our heroes will lose from teams U najgorem će slučaju na&scaron; trojac <code>NisamSadaNistaDonio</code> and <code>JeLiMojKockaSeUmio</code>, thereby finishing in third place.</p>