...
시간 제한: 1.122 초, 메모리 제한: 1122 MB

### 통계

제출: 1587, 정답: 336, 맞힌 사람: 286, 정답 비율: 22.467%
...
<ol>
	<li>구간 $\left[-10^{18}, 10^{18}\right]$에 있는 단 하나의 정수만 모든 질의응답에 대해 참이라면 준성이의 최애 숫자가 무엇인지 정확히 알아낼 수 있다고 한다.</li>
	<li>구간 $\left[-10^{18}, 10^{18}\right]$에 있는 모든 정수가 모든 질의응답에 대해 거짓이라면 질의응답에 모순된 내용이 있다고 한다.</li>
</ol>