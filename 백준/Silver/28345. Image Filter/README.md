...
제출: 161, 정답: 119, 맞힌 사람: 85, 정답 비율: 73.913%
...
<p>여기서 우측 하단의 &lsquo;<code>1</code>&rsquo;을 표현하는 경우를 살펴보면,</p>
...
<p>각 테스트 케이스의 첫 줄에는 높이맵의 크기 $R$, $C$ ($1 &le; R &le; 100$, $1 &le; C &le; 100$)가 공백으로 구분되어 주어진다. 높이맵의 크기는 $R$행 $C$열이다. 이후 $R$개의 줄에, 높이맵의 픽셀의 값을 나타내는 $C$개의 정수 $H_{i,j}$ ($0 &le; H_{i,j} &lt; 2^{16}$)들이 빈 칸을 사이에 두고 주어진다.</p>
