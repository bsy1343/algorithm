...
제출: 2843, 정답: 644, 맞힌 사람: 550, 정답 비율: 26.153%
...
<p>코로나 19이후 우울함에 빠져 있는 소년 짐(Jim)은 세상의 모든 짐을 지고 있는 느낌이다. 짐은 친구들의 행복 정도를 측정하고 싶어졌다. 짐이 생각한 한 가지 방법은 친구들이 보낸 문자메시지를 분석하여 친구들이 행복한지 판단하는 것이다. 문자 메시지의 글자가 단어 &ldquo;<code>HAPPY</code>&rdquo;에 나타나면 그 글자는 행복한 글자이고 단어 &ldquo;<code>SAD</code>&rdquo;에 나타나면 그 글자는 우울한 글자이다. 글자 &lsquo;<code>A</code>&rsquo;는 양쪽에 모두 나타나므로 행복하기도 하고 우울하기도 한 글자이다. 글자 &lsquo;<code>B</code>&rsquo;는 어느 쪽에도 나타나지 않으므로 행복하지도 않고 우울하지도 않은 글자이다.</p>
...
<blockquote>
<p><code>&ldquo;SAD MOVIES ALWAYS MAKE ME CRY&rdquo;</code></p>
</blockquote>
