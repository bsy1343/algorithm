...
제출: 551, 정답: 262, 맞힌 사람: 236, 정답 비율: 49.476%
...
<p>① $\lfloor x \rfloor$는 $x$보다 작거나 같은 정수 중 가장 큰 값을 의미한다.</p>

<p>② "사전 순" 정렬 기준: 문자열을 사전 순으로 정렬할 때는 다음 기준에 따라 순서를 결정한다.</p>

<ol>
	<li>문자열을 정렬할 때, 앞 글자부터 차례대로 비교하며 알파벳 순서 상 앞선 글자가 먼저 오는 순서가 우선이다. </li>
	<li>비교 중인 글자가 같을 경우, 그다음 글자를 계속 비교하여 순서를 결정한다.</li>
	<li>문자열의 길이가 다를 경우, 앞부분이 모두 같다면 짧은 문자열이 먼저 온다. (예: "ab" → "abc")</li>
</ol>