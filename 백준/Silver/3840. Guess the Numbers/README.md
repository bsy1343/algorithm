...
<p>However, no sooner had he resolved to pay attention than the lesson ended. So the only thing he has been able to do is to hurriedly copy the content of the blackboard in his notebook. Today, the teacher was explaining basic arithmetic expressions with unknowns. He vaguely remembers that his classmates have been substituting values into the unknowns to obtain the expressions&rsquo; results. However, in all the hurry, John has only written down expressions, values and results in a messy fashion. So he does not know which value comes with each unknown, or which result goes with each expression.</p>
...
<ul>
	<li>The first line contains a sequence of natural numbers. The first one (1 &le; n &le; 5) is the number of unknowns that will occur in the expression. It is followed by a sequence of n integers v1 . . . vn (0 &le; vi &le; 50), which are the values to be assigned to the unknowns. Finally, there is an integer m (0 &le; m &le; 1000) representing the desired result of the evaluation of the expression.</li>
