...
제출: 55, 정답: 27, 맞힌 사람: 23, 정답 비율: 50.000%
...
<p>The first line contains four numbers s,w,p,t. 1 &le; s &le; 50 is the number of suspects, 1 &le; w &le; 200 the number of witnesses, 1 &le; p &le; 50 the number of possible locations, and 0 &le; t &le; 1000 the time of the crime (all are integers).</p>

<p>This is followed by w lines, each describing one witness statement by giving four numbers Si,Pi,bi,fi. 1 &le; Si &le; s is the number of the suspect the witness claims to have seen, 1 &le; Pi &le; p the place at which the witness claims to have been, and [bi,fi] the claimed time interval of the observation. All these numbers are integers.</p>

### 출력

<p>For each data set, output &ldquo;Data Set x:&rdquo; on a line by itself, where x is its number. On the subsequent lines, output, one per line, the suspects who are still under suspicion, i.e., the ones without valid alibis. If all suspects have alibis, output &ldquo;No suspect.&rdquo; instead. They should be output in sorted order. Each data set should be followed by an empty line.</p>