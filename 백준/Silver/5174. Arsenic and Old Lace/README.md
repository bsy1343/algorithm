...
제출: 50, 정답: 26, 맞힌 사람: 20, 정답 비율: 48.780%
...
<p>The first line of each data set contains two numbers s,b, the number of individual substances 1 &le; s &le; 50, and the number of base products 1 &le; b &le; 20. This is followed by b lines, each describing a base product with a string of s letters. The ith character is &lsquo;y&rsquo; if the base product contains the individual substance i, and &lsquo;n&rsquo; otherwise. Finally, one more line describes the poison in the victim in the same way.</p>

### 출력

<p>For each data set, output &ldquo;Data Set x:&rdquo; on a line by itself, where x is its number. On the next line, output the minimum number of base substances that together would provide all the individual substances in the poison, but no more. If there is no such combination, output &ldquo;Impossible.&rdquo; instead. Each data set should be followed by a blank line.</p>