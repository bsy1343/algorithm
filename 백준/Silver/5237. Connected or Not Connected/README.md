...
제출: 370, 정답: 248, 맞힌 사람: 200, 정답 비율: 68.966%
...
<p>The first line in the test data file contains the number of test cases (&le; 50). After that, each line contains a test case. The first number in each test case is the number of sites, n (&le; 100). The sites are numbered from 0 to n &minus; 1. The second number is the number of connections between the sites, k (&le; 200). After that there are k pairs of numbers, one for each connection. The connections might repeat and further, there may be self-connections (i.e., a connection from a site to itself).</p>

### 출력

<p>For each test case, you are to check whether there is path from every site to every other site, and output &ldquo;Connected.&rdquo; or &ldquo;Not connected.&rdquo; accordingly.</p>