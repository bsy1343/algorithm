...
제출: 99, 정답: 53, 맞힌 사람: 51, 정답 비율: 55.435%
...
<p>The input file contains several test cases, in which the actual year has to be calculated. The description of each case starts with a line containing an integer n (1 &le; n &le; 20), the number of computers. Then, there is one line containing three integers y<sub>i</sub>,a<sub>i</sub>,b<sub>i</sub> for each computer (0 &le; a<sub>i</sub> &le; y<sub>i</sub> &lt; b<sub>i</sub> &lt; 10000). y<sub>i</sub> is the year the computer displays, b<sub>i</sub> is the year in which the bug happens (i. e. the first year which can&#39;t be displayed by this computer) and a<sub>i</sub> is the year that the computer displays instead of b<sub>i</sub>.&nbsp;</p>
