# [Silver IV] Ballot evaluation - 6495
...
제출: 305, 정답: 88, 맞힌 사람: 72, 정답 비율: 29.752%

### 분류

구현, 자료 구조, 문자열, 집합과 맵, 해시를 사용한 집합과 맵, 파싱, 임의 정밀도 / 큰 수 연산
...
<p>The data provided by Bill and Ted has the following format: The first line consists of the number <em>p</em> of parties followed by the number <em>g</em> of guesses (with <em>1 &le; p &le; 50</em> and <em>1 &le; g &le; 10000</em>). Then follow <em>p</em> lines, each line consisting of a unique party name of length &le; 20 (only containing letters a-z, A-Z and digits 0-9) and the achieved vote percentage of this party with one digit after the decimal point. After the parties follow <em>g</em> lines, each consisting of a guess. A guess has the form <em>P<sub>1</sub> + P<sub>2</sub> + ... + P<sub>k</sub> COMP n</em>, where <em>P<sub>1</sub></em> to <em>P<sub>k</sub></em> are party names, <em>COMP</em> is one of the comparison operators &lt;, &gt;, &lt;=, &gt;= or = and <em>n</em> is an integer between 0 and 100, inclusively. Each party name occurs at most once in each guess.</p>
