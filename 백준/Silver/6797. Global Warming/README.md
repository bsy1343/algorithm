...
제출: 119, 정답: 46, 맞힌 사람: 38, 정답 비율: 38.384%
...
<p>then we can calculate that the temperature changes first 1 tenth up, then 2 up, then 2 down, 1 up, 2 up, and 2 down. There is a cycle of changes of length three which covers all of the temperature differences: (+1, +2, &minus;2). In other words, if we look at the differences starting at the first position, there is a cycle of length three of the form (+1, +2, &minus;2) followed by another cycle of length three of exactly the same form.</p>
...
<p>The input consists of a number of test cases. Each test case starts with the number n (1 &le; n &le; 20), representing the number of temperatures in a sequence, followed by the sequence of n temperatures. You may assume that each temperature input is an integer in the range &minus;1000...1000 inclusive. The numbers are separated by a single space. The last test case is indicated by a zero and should produce no output.</p>
