...
제출: 54, 정답: 36, 맞힌 사람: 31, 정답 비율: 67.391%
...
<p>One may define a map of strings over an alphabet \(\Sigma_B = \{C_1, C_2, \dots, C_B\}\) of size $B$ to non-negative integer numbers, using characters as digits $C_1 = 0, C_2 = 1, \dots , C_B = B &minus; 1$ and interpreting the string as the representation of some number in a scale of notation with base $B$. Let us denote this map by $U_B$, for a string $\alpha[1..n]$ of length $n$ we put&nbsp;\[U_B(\alpha) = \sum_{i=0}^{n-1}{\alpha[n-i]\cdot B^i}\text{.}\]</p>
