...
제출: 157, 정답: 100, 맞힌 사람: 96, 정답 비율: 73.846%
...
<ul>
	...
	<li>Cells with adjacent sides on a common pixel that contains a 1, comprise the canopy layer. A single large canopy layer image will contain all 1&rsquo;s.&nbsp;</li>
...
<p>For each test case, produce a single line of output that starts with the prefix &ldquo;Case #x:&rdquo; where x represents the case number (starting from one and incrementing at each new test case), followed by a single space, and then the result, i.e., the number of canopy layers in the spot image.</p>